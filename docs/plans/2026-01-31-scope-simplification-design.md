...
**Date**: 2026-01-31 **Status**: ✅ **COMPLETED** (Design → Execution → Release)
...
>
> This design document guided the scope simplification that resulted in v0.2.0. All
...
>
> **See:** `2026-01-31-remove-notifications-and-blueprints.md` for detailed
> implementation plan
>
> **Result:** Integration now focuses exclusively on core light control with
> verification/retry and preset management.
...
✅ Core light control with verification/retry ✅ Preset management (CRUD operations) ✅
Config flow UI with collapsible sections ✅ Button/sensor entities for presets ✅
Exponential backoff retry strategy ❌ Notification system (`notify_on_failure`) - **TO
REMOVE** ❌ Blueprint automations (4 files) - **TO REMOVE**
...
- `custom_components/ha_light_controller/config_flow.py`
  - Remove: `CONF_NOTIFY_ON_FAILURE` import and all references (lines 32, 115, 192-193,
    363-364)
...
**Current state**: Services registered in `async_setup_entry()` ✅ **Best practice**:
Services should be registered at integration level (see
[Service docs](https://developers.home-assistant.io/docs/dev_101_services))

**Finding**: Current implementation is correct for integration-level services tied to a
config entry.
...
relying on notifications can implement them via automations triggered by service
responses.
```
...
- ✅ Core `ensure_state` service works with all existing parameters (minus
  notify_on_failure)
