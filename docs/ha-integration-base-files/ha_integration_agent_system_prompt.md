...
## ⚠️ Critical: Home Assistant Environment Constraints

**Blocking calls freeze the ENTIRE Home Assistant instance.** See `ha_integration_agent_spec.md` Section 3 for details.

| Rule | Requirement |
|------|-------------|
| Async | All I/O via async/await or `hass.async_add_executor_job()`. Never `time.sleep()` or sync `requests` |
| Resources | Poll 30-60s for local, respect cloud rate limits. Prefer listeners over polling |
| Sandbox | No filesystem outside `config/`. All deps via PyPI + `manifest.json` |
| Library separation | Device comms in separate library (mandatory for core, recommended for HACS) |
| State access | Use `hass.services.async_call()`, `hass.states.get()`. Never bypass HA's state machine |
