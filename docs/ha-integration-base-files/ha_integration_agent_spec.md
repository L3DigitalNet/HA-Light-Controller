...
## 3. Home Assistant Environment Constraints

### 3.1 Single-Threaded Event Loop

**Blocking calls freeze the ENTIRE Home Assistant instance.** Even 1-second blocks cause unresponsive UI, failed automations, and watchdog restarts.
```python
...
def get_device_data():
    time.sleep(1)  # Blocks HA for 1 second
...
# ✅ CORRECT - wrapping sync libraries
async def async_get_device_data(hass):
    # Runs sync code in thread pool executor
    return await hass.async_add_executor_job(sync_library.get_data)
```

### 3.2 Resource Constraints

HA runs on RPi-class hardware. Poll 30-60s for local devices, respect cloud rate limits. Use `DataUpdateCoordinator`. Prefer listeners over polling.

### 3.3 Sandboxed Execution

No filesystem access outside `config/`. No system packages or binaries. All deps via PyPI + `manifest.json`.

### 3.4 Library Separation

Device comms must be in a **separate Python library** (mandatory for core, recommended for HACS). Integration only handles HA-specific logic. Declare in `manifest.json` under `requirements`.

---

## 4. Critical Development Patterns

### 4.1 The DataUpdateCoordinator Pattern
...
### 4.2 Config Flow Implementation
...
## 5. manifest.json Complete Reference
...
### 6.1 Minimum Test Requirements (Bronze Tier)
...
See Section 3 for event loop blocking (the most critical pitfall).

### 7.1 Not Handling Unavailability
...
### 7.3 Hardcoded Polling Intervals
...
### 8.1 Recommended Approach: Dev Container
...
### 8.2 Alternative: Local Virtual Environment
...
## 9. Agent Workflow: Planning Phase
...
## 10. Agent Response Templates

### 10.1 New Integration Start
